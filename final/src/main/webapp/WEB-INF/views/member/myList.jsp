...
<body>
  <c:if test="${board_type == 3}">
    <c:if test="${count == 0}">
      <jsp:include
        page="/WEB-INF/views/member/myListLocation.jsp"
        flush="false"
      /><br />
    ...
    <c:forEach var="board" items="${boardList}">
      <tr>
        ...
        <c:if test="${board.board_type == 3}">
          <td>
            <a
              href="/help/qna/qnaContent?pnum=${board.pnum}&pageNum=${currentPage}"
              >${board.subject}</a
            >
          </td>
        </c:if>
        <c:if test="${board.board_type == 4}">
          <td>
            <a
              href="/community/review/reviewContent?pnum=${board.pnum}&pageNum=${currentPage}"
              >${board.subject}</a
            >
          </td>
        </c:if>
        <c:if test="${board.board_type == 5}">
          <td>
            <a
              href="/community/question/questionContent?pnum=${board.pnum}&pageNum=${currentPage}"
              >${board.subject}</a
            >
          </td>
        </c:if>
        <c:if test="${board.board_type == 6}">
          <td>
            <a
              href="/community/info/infoContent?pnum=${board.pnum}&pageNum=${currentPage}"
              >${board.subject}</a
            >
          </td>
        </c:if>
        <c:if test="${board.board_type == 7}">
          <td>
            <a
              href="/community/job_seeker/job_seekerContent?pnum=${board.pnum}&pageNum=${currentPage}"
              >${board.subject}</a
            >
          </td>
        </c:if>
  ...
  <c:if test="${count > 0}">
    ...
    <c:if test="${startPage > 10}">
      <a
        href="/member/myList?pageNum=${startPage - 10}&board_type=${board_type}&writer=${sessionScope.sid}"
        >[이전]</a
      >
    </c:if>
    <c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
      <a
        href="/member/myList?pageNum=${i}&board_type=${board_type}&writer=${sessionScope.sid}"
        >[${i}]</a
      >
    </c:forEach>

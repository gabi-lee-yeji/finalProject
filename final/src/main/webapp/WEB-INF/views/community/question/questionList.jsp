	...
	<section style="margin-left:5%;margin-right:5%;margin-bottom:5%">
		...
		<c:if test="${count == 0}">
			<table class="table table-hover">
				<thead>
					<tr>
						<td><h3>질문글이 없습니다. 다른 회원에게 묻고싶은 질문을 올려주세요!</h3></td>
					</tr>
				</thead>
			</table>
		</c:if>
		...
		<form action="/community/question/searchList" method="get">
			<div class="row">
		...
		<c:if test="${count > 0}">
			...
			<ul class="pagination justify-content-center">
				<c:if test="${startPage > 10}" >
					<li class="page-item">
			        	<a class="page-link" href="/community/question/questionList?board_type=5&pageNum=${startPage - 10}">이전</a>
			    	</li>
			    </c:if>
			    
			    <c:forEach var="i" begin="${startPage}" end="${endPage}" step="1">
			    	<li class="page-item">    
			        	<a class="page-link" href="/community/question/questionList?board_type=5&pageNum=${i}">${i}</a>
			    	</li>
			    </c:forEach>
			    
			    <c:if test="${endPage < pageCount}" >
			    	<li class="page-item">
			       		<a class="page-link" href="/community/question/questionList?board_type=5&pageNum=${startPage + 10 }">다음</a>
			    	</li>
			    </c:if>
			</ul>
		</c:if>
